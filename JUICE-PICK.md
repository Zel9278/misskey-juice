...
- ~~現時点では ActivityPub での AI フラグ連合は行わない。~~ → 方針転換: `_juice_isAIGenerated` というJUICE独自のASプロパティ(`misskey:`とは別の`juice:`名前空間)で連合するように実装した。未対応サーバーからは単に無視される(受信側は未指定なら`false`扱い)。ノート単位のみで、Driveファイル単位・投稿後編集の連合は対象外。
