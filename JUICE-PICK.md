...
- 現時点では ActivityPub での AI フラグ連合は行わない。
...
- AI 生成物ミュートの適用範囲（タイムラインのみか、通知・検索・ユーザーページなども含むか）。
- AI 生成物ミュート時に、AI 生成物だけを完全に非表示にするか、折りたたみ表示にするか。
- About ページの詳細な紹介文。
- AI フラグの引用ノート時の UI 表示詳細。フラグ自体は元ノートの値をそのまま保持する。
...
- AI Generated（AI生成）バッジを表示し、ActivityPub 連合は行わない。リノートでは表示を強制し、編集では変更可能とする。引用ノートでは元ノートのフラグをそのまま保持する。
