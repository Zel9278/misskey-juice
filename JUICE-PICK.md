...
- `/settings/juice` への専用トグル追加という案(実装方針メモの案)ではなく、ウィジェット編集UI内の個別ボタンとして実装した。編集対象がウィジェット単位である以上、既存の設定/削除ボタンと同じ場所に置くほうが導線として自然と判断したため。`place` 未設定(null)のウィジェットは「右寄せ」と同じ扱いにしている(既存のフィルタ条件 `place !== 'left'` と一致させるため、三値ではなく二値のトグルにした)。
- Playwrightでの実機確認: ウィジェットをトグルすると即座にドロワー内の表示順が変わる(末尾のウィジェットを左寄せにすると先頭へ移動)ことを確認済み。
