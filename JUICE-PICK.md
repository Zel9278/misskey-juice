...
## 7. モバイル表示時のウィジェット並び順設定（JUICE 独自実装） ✅ 完了
...
### リレータイムライン(5.)に特定リレー1件への絞り込みフィルタを追加(2026-08-30)

- 従来はリレータイムラインが受理済み全リレー経由のノートをまとめて表示するのみで、特定のリレー1件だけに絞り込む手段が無かった。
- `notes/relay-timeline`(REST)・`relayTimeline`(stream channel)双方に任意の`relayId`パラメータを追加し、指定した場合は`note.relayId`が一致するノートのみを返すように。フィルタ選択肢を組み立てるための新規エンドポイント`juice/relays`(受理済み=`status: 'accepted'`のリレーのみを`{id, host}`で返す)も追加した。
- フロントエンドはリレー選択専用のUIを新設せず、既存のタイムラインヘッダーの「…」オプションメニューに`type: 'radio'`のメニュー項目(「表示するリレー」、選択肢は「すべてのリレー」+登録済み各リレーのホスト名)として追加した。ユーザーの手持ちリスト向けの`src`文字列エンコード方式(`list:${listId}`のような`:`区切り)は、リレーがインスタンス管理者による少数の固定リストではなく可変長かつ管理者裁量である点を踏まえて採用せず、ページローカルな非永続の`ref`(`selectedRelayId`)に留めた。
