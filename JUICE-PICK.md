...
## 7. モバイル表示時のウィジェット表示位置(左右)設定（JUICE 独自実装） ✅ 完了
...
- ユーザーごとの個人設定として、モバイル表示時に各ウィジェットを左右どちらに表示するか(位置)を選べるようにする。ウィジェット同士の並び順自体は既存のドラッグ&ドロップ並び替えで既に設定できるため、この機能では対象にしない。
- インスタンス全体の管理者設定(コントロールパネルの「JUICE」項目)ではなく、ユーザー個人の設定画面(メール受信言語設定と同じ枠の `/settings/juice` など)から変更可能にする。
- 実装候補: 新規トグルを増やすのではなく、既存の `place` 属性(ウィジェットごとに左/右を個別指定できる仕組み)をモバイルドロワーでも参照し、指定に応じて表示位置を決める方向で検討する。
- 具体的な位置ロジック・UI文言・`place` 未設定ウィジェットの扱いは実装着手時に詰める。

### 実装結果(2026-08-29〜2026-08-30)
...
- 当初はウィジェット編集UI内の個別ボタンのみで実装したが、ユーザーから「JUICE側(`/settings/juice`)にも生やしてほしい」との要望を受け、`packages/frontend/src/pages/settings/juice.vue` に「モバイルウィジェットの表示位置」セクションを追加した。中身はユーザーの現在のウィジェット一覧を `MkSwitch` で列挙し、ON/OFFが `place: 'left'/'right'` にそのまま対応する。ウィジェット編集モードの切り替えボタンと同じ `prefer.commit('widgets', ...)` を直接呼び出しており、保存先は完全に同一(片方で変更すればもう片方にも即座に反映される)。`place` 未設定(null)のウィジェットは「右寄せ」と同じ扱いにしている(既存のフィルタ条件 `place !== 'left'` と一致させるため、三値ではなく二値のトグルにした)。
...
- レビューで指摘(2026-08-30): ロードマップ・設定画面の呼称が「並び順」になっていたが、この機能で実際に変更できるのは各ウィジェットを左右どちらに表示するかという**位置**のみで、同じ側にまとまったウィジェット同士の並び順自体は既存のドラッグ&ドロップ並び替えで変更できる(設定画面に別途並び替えUIを設ける必要はない)。呼称を「表示位置(左右)」に統一した(表示文言のみの変更で、`widgetPlace`等のi18nキー名・実装ロジックは変更なし)。
...
- 本ロードマップの項目3(絵文字申請)・7(ウィジェット表示位置、設定ページでの一覧UIの参照パターンとして)・8(お知らせの投票機能)は、いずれもこの機能が持つ「ユーザー×お知らせ」中間テーブルパターン(`(userId, announcementId, 識別子)`ユニーク複合インデックス)を直接のテンプレートとして参照・踏襲している。
...
- 初期実装は単一リレーのみを選べる`relayId`(単数)+タイムラインヘッダーの「…」オプションメニューの`type: 'radio'`項目+ページローカルな非永続の`ref`という構成だったが、レビューで「複数選択の方が良い」「JUICEの設定(`/settings/juice`)からも設定できるようにしたい」との指摘を受け、以下の構成に作り直した。
  ...
  - `/settings/juice`にも「表示するリレー」セクションを追加し、リレー一覧を`MkSwitch`で列挙(ウィジェット表示位置設定と同じ構成)。ヘッダーメニューと設定ページはどちらも同じ`prefer.s.relayTimelineFilter`を読み書きするため、片方で変更すればもう片方にも即座に反映される。
