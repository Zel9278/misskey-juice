...
### 承認式登録の審査状況確認「サインインする」ボタンの不具合修正(2026-08-30)

- ユーザー報告により、承認済みエントリの「サインインする」ボタンが2つの独立した理由で機能していなかったことが判明した。
  1. **z-index優先度の不一致**: このボタンが`MkSignupCheckPanel.vue`(画面右からスライドインするパネル、`claimZIndex('high')`を直接使用)の中で押された場合、開かれる`MkSigninDialog.vue`が`MkModal`の`zPriority`を指定しておらず既定の`'low'`のままだったため、ダイアログがパネルの裏に隠れて見た目上反応しないように見えた。`claimZIndex`は優先度ごとに別々の単調増加カウンタ(`low`は100万台、`high`は300万台)のため、優先度が違えば呼び出し順に関わらず`high`が常に上に来る。隣接する`MkDialog.vue`/`MkWaitingDialog.vue`/`MkSignupApprovalPendingDialog.vue`が既に`:zPriority="'high'"`を指定していたのに`MkSigninDialog.vue`だけ抜けていた、という単純な指定漏れだったため、同じ`'high'`を追加して解消した。
  2. **より根本的な問題**: `pleaseLogin()`(`packages/frontend/src/utility/please-login.ts`)は「既に何らかのアカウントでログイン中なら何もしない」設計のガード用ユーティリティで、既存の操作(リアクション等)を未ログイン時にだけ保護する用途を想定している。しかし審査状況確認画面は「ブラウザは別の既存アカウントで既にログイン中だが、まだ承認された新アカウントにはログインしていない」という状況が普通にあり得るため、この設計と噛み合わず、ログイン中はボタンを押しても無反応だった。`MkVisitorDashboard.vue`の「ログイン」ボタン(`signin()`)と同じパターン(`os.popup(XSigninDialog, { autoSet: true }, { closed: () => dispose() })`でログイン状態に関わらず常時ダイアログを開く)に置き換えて解消した。
- 副次的な改善として、ボタン文言を「サインインする」から「サインイン画面を開く」に変更した。確認コードにはユーザー名・パスワードが一切紐づいておらず(プライバシー上、コードだけでどのアカウントか特定できない設計)、押しても自動ログインはされずサインインダイアログが開くだけのため、元の文言だと自動ログインされるかのように誤解されやすかった。

### 本家画面に混ざったJUICE独自要素への「JUICE」バッジ表示(2026-08-30)

- 本家(upstream)のUIの中にJUICE独自の設定項目・タブ・メニュー項目が見分けのつかない形で混ざっており、どれが本家標準機能でどれがフォーク固有か画面上で区別できなかったため、ユーザーの依頼で色付きの「JUICE」バッジを追加した。
- 既存の`._beta`/`._modified`(`packages/frontend/src/style.scss`)と同じボックスモデルで`._juice`クラスを新設した(色は`accent`/`warn`と被らないよう`success`を使用)。バッジ文字列"JUICE"はブランド名としてi18nキー化せずハードコードしている。
- 設定項目は`<template #label>`直後に`<span class="_juice">JUICE</span>`を差し込むだけで対応できたが、タブ(`MkPageHeader.tabs.vue`の`Tab`型)とメニュー項目(`types/menu.ts`の`TextMenuBase`)はラベルが単純な文字列としてのみ描画される共通部品だったため、両方に`badge?: boolean`フィールドを追加し、共通部品自体を拡張する方針にした(影響範囲は広がるが、タブ・メニューでも設定項目と同じ見た目のバッジを出せる)。iconOnlyなタブは非アクティブ時は小さいドットのみ表示し、アクティブになるとタイトルと一緒にバッジ本体を表示する。
- **JUICE独自かどうかの判定方法を、この作業の途中で修正した**: 当初は「コミットメッセージが日本語で、upstream PR番号が付いていない」ことを根拠にロール設定の`watermarkAvailable`(ウォーターマーク機能をロールで制御する設定、ドライブ設定のウォーターマーク編集セクション含む)もJUICE独自と判断してバッジを付けていたが、ユーザーからの指摘を受けて`git merge-base --is-ancestor <commit> upstream/develop`で機械的に検証し直したところ、該当コミットが実際にはupstream/developの祖先コミットだった(syuilo本人がPRを介さず直接コミットした本家の変更だった)ことが判明し、対象から除外した。以降は全て`merge-base --is-ancestor`で検証し、`emojiRequestLimit`ロールポリシー・お知らせの投票機能・探索ページの「ユーザーランキング」タブ・タイムラインの「リレー」タブ/絞り込みメニュー・ロゴメニューの「絵文字申請」リンクの5箇所を、genuine にJUICE独自と確定させた。
- 実装後に2件の不具合が見つかり修正した。
  - **バッジが浮いて見える問題**: `._juice`の`vertical-align: top`が、タブ・メニューの`display: flex`な行や、ロール設定のラベルをラップする`MkCondensedLine`(`display: inline-block`+`transform: scaleX()`で長い文言を自動縮小する共通コンポーネント)の中では、バッジがラベルから浮いて見える(ラベルに「付いていない」ように見える)原因になっていた。個別のコンポーネントごとに上書きする対症療法ではなく、`._juice`自体の既定値を`top`から`middle`に変更する根本修正に切り替えた。
  - **アイコン無しタブへの余白混入**: `MkPageHeader.tabs.vue`でアイコン用のラッパー`span`を無条件でレンダリングするよう実装してしまい、アイコンを持たないタブ(`explore.vue`の「Featured」「Users」「Roles」等)にも意図しない`padding-left`が付くレイアウト崩れが発生していた。`vue-component-reviewer`のレビューで指摘され、ラッパーに`v-if="t.icon"`を追加して修正した。
  - **メニュー項目`type: 'link'`にバッジが表示されない問題**: `MkMenu.vue`側は当初`type: 'parent'`のブロックにしかバッジ描画を実装していなかったため、ロゴクリックで開く本家標準のインスタンスメニュー(`packages/frontend/src/ui/_common_/common.ts`)に混ざっていたJUICE独自の「絵文字申請」リンク(`type: 'link'`)には`badge: true`を設定してもバッジが表示されていなかった。ユーザー報告で発覚し、`type: 'link'`のブロックにも同じバッジ描画を追加した。

### JUICE専用Aboutページに「オレンジの雨」イースターエッグを追加(2026-08-30)

- 本家Misskeyの`about-misskey.vue`には「アイコンクリックで絵文字が重力落下する」有名なイースターエッグ(Matter.jsベースの物理エンジン`utility/physics.ts`を使用)がある。当初はこれをそのまま`about-juice.vue`に移植したが、ユーザーから「絵文字じゃなくてオレンジの細い線が雨みたいに降る感じにしてほしい」というフィードバックを受け、方針を変更した。
- 物理エンジンは使わず、純粋なCSS `@keyframes`アニメーションのみで実装し直した。クリックのたびに`rainBurstId`という単調増加カウンタを進めて40個の雨粒(`rainDrops`)を丸ごと再生成することで、アニメーション再生中に連打しても要素が確実に再マウントされ、最初から降り直すようにしている(同じkeyのまま値だけ更新すると、ブラウザによってはCSSアニメーションが正しく再スタートしないことがあるため)。
- 「押すたびに角度が変わっても良い」という要望に対応するため、`--rain-angle`というCSSカスタムプロパティをクリックのたびにランダム(±25度)に設定し、keyframe内で`rotate(var(--rain-angle)) translateY(...)`の順で合成した。`rotate`を先に適用することで、以降の`translateY`が回転済みの座標系に沿って動くため、線自体の傾きと落下方向が一致する(JSでsin/cosを計算してdx/dyを求める必要が無い)。
- レビュー指摘への対応: クリックトリガーが装飾目的の`<img>`だったため、新規実装(本家からの機械的な移植ではない)としてキーボード操作に対応させる必要があると判断し、`role="button"` `tabindex="0"` `@keydown.enter` `@keydown.space.prevent`を追加した。雨の色(`#f2841f`)は`--MI_THEME-accent`等のテーマ変数だと既定のdarkテーマで緑になってしまい「オレンジの雨」という意図が崩れるため、テーマに依存しないJUICEブランド色として`$juice-rain-color`という名前付きSCSS変数で明示した(`MkChart.vue`がグラフの色を固定色でハードコードしている前例に倣った)。
- ユーザーの追加要望で、雨を降らせると解除される実績`juiceRain`(「JUICEの雨」、🍊アイコン、bronzeフレーム)を追加した。Misskeyの実績システムは`ACHIEVEMENT_TYPES`という`as const`配列(`packages/backend/src/models/UserProfile.ts`)への追加だけでマイグレーション不要(`achievements`列自体は元々jsonb型)だが、フロントエンド側`packages/frontend/src/utility/achievements.ts`にも同じ配列とバッジ定義(`ACHIEVEMENT_BADGES`)が独立して重複定義されている(本家の既存構造で、JUICEが導入したものではない)ため両方に追加が必要だった。`i/claim-achievement`エンドポイントの`paramDef`が`ACHIEVEMENT_TYPES`をそのまま`enum`に使っているため、`misskey-js`の`AchievementName`型に新しい実績名を反映させるための再生成(`pnpm build-misskey-js-with-types`)も必要だった。管理者アカウントを用意できず実際の実績解除通知・実績一覧表示までは実機確認できていないが、`about-misskey.vue`の`getTreasure()`(`claimAchievement('foundTreasure')`)と全く同じ呼び出しパターンを踏襲しているため、ロジック上のリスクは低いと判断した。
