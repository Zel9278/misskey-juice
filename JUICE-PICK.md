...
- `packages/frontend/src/components/MkWidgets.vue` の編集モードに、ウィジェットごとの左寄せ/右寄せ切り替えボタン(`placeEditable` プロパティで表示制御)を追加。既存の設定・削除ボタンと同じ並びに配置し、クリックで `place` を `'left'`/`'right'` にトグルして `updateWidgets` イベント経由で保存する(新規イベント種別は追加せず、既存の並び替え保存経路をそのまま再利用)。
- `MkWidgets.vue` に `sortByPlace` プロパティを追加し、表示専用(編集モード以外)の描画リストでのみ `place === 'left'` のウィジェットを先頭に安定ソートする(相対順序は維持)。`ui/_common_/widgets.vue` はこのプロパティをそのまま下へ渡すだけで、自身の `widgets` computed(ドラッグ並び替え・保存の基準になる配列)はソートしない。
  - レビューで指摘: 最初の実装では `ui/_common_/widgets.vue` 側の `widgets` computed 自体をソートしていたため、その配列がそのまま編集モードの `MkDraggable` とドラッグ後の保存(`updateWidgets`)にも使われ、place切り替えボタンを1回押しただけで全ウィジェットの保存順が左右グループ順に組み替えられてしまう不具合があった。デスクトップの常設パネル(`universal.vue`)とも同じ保存済み配列を共有しているため、モバイルでの操作がデスクトップの表示順まで壊しうる状態だった。ソート処理を表示専用の描画リストにのみ限定することで修正し、Playwrightで保存順(localStorageの`preferences.widgets`)が操作前後で変わらないことを確認済み。
- モバイル/狭幅画面のウィジェットドロワー(`packages/frontend/src/ui/_common_/common.vue` の `widgetsDrawer`)にのみ `sortByPlace`/`placeEditable` を有効化。デスクトップの常設ウィジェットパネル(`universal.vue`)は挙動変更なし(既存どおり登録順)。
- 当初はウィジェット編集UI内の個別ボタンのみで実装したが、ユーザーから「JUICE側(`/settings/juice`)にも生やしてほしい」との要望を受け、`packages/frontend/src/pages/settings/juice.vue` に「モバイルウィジェットの並び順」セクションを追加した。中身はユーザーの現在のウィジェット一覧を `MkSwitch` で列挙し、ON/OFFが `place: 'left'/'right'` にそのまま対応する。ウィジェット編集モードの切り替えボタンと同じ `prefer.commit('widgets', ...)` を直接呼び出しており、保存先は完全に同一(片方で変更すればもう片方にも即座に反映される)。`place` 未設定(null)のウィジェットは「右寄せ」と同じ扱いにしている(既存のフィルタ条件 `place !== 'left'` と一致させるため、三値ではなく二値のトグルにした)。
