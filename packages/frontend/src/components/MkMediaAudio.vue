...
>
	...
	<div
		...
		v-panel
		role="button"
		:aria-label="inlinePlayable ? i18n.ts.audio : undefined"
		tabindex="0"
	...
	>
		<!-- JUICE: メディアタイムラインでは拡大表示を経由せずその場で再生できるようにする。
		     ライトボックスと同じビジュアライザー(XAudioVisualizer)+コントロール(XControl)を再利用する -->
		<template v-if="inlinePlayable">
			...
			/>
			<!-- JUICE: 親(.audioRoot)のEnter/Spaceキー操作(再生/一時停止トグル)が、ここに入っている
			     XControl自身のボタン・スライダーへのキーボード操作を奪ってしまわないようkeydownを止める -->
			<div :class="$style.inlineControls" @pointerdown.stop @click.stop @keydown.stop>
				<XControl ref="mediaControl" v-model:volume="volume" :expandAction="onExpandAction"/>
			</div>
		</template>
		<template v-else>
		...
		<button :class="[$style.menu, inlinePlayable ? $style.menuTopLeft : $style.menuBottom]" class="_button" @click.stop="showMenu" @keydown.stop><i class="ti ti-dots" style="vertical-align: middle;" aria-hidden="true"></i></button>
...
import type XAudioVisualizer__TypeReferenceOnly from '@/components/MkLightbox.item.audio-visualizer.vue';
...
// JUICE: ライトボックスと同じビジュアライザーをメディアタイムラインのインライン再生でも使う。
// canvas描画やAudioContext周りの依存を、非表示時・非インライン時には読み込ませないよう遅延importする
const XAudioVisualizer = defineAsyncComponent(() => import('@/components/MkLightbox.item.audio-visualizer.vue'));
...
function onRootClick(ev: PointerEvent | KeyboardEvent) {
	if (props.inlinePlayable) {
		// JUICE: <audio>本体はcontrols無しでは表示領域を持たない(クリックを受け取れない)ため、
		// 見た目のカード全体(波形アート等)のクリックをここで再生/一時停止に割り当てる
		togglePlayPause();
	} else {
		emit('mediaClick', ev);
	}
...
// JUICE: インライン再生用。ライトボックスと同じXAudioVisualizerが内部で持つ<audio>要素を、
// 同じXControlを再利用するために同じDIキー(DI.mkLightboxItemMediaEl)へprovideする
const audioVisualizer = useTemplateRef<InstanceType<typeof XAudioVisualizer__TypeReferenceOnly>>('audioVisualizer');
...
const visualizerContent = computed<Content>(() => ({
	id: props.audio.id,
	type: 'audio',
	url: props.audio.url,
	file: props.audio,
...
provide(DI.mkLightboxItemMediaEl, computed(() => audioVisualizer.value?.audioEl ?? null));

const isMediaPlaying = computed(() => mediaControl.value?.isPlaying ?? false);

function togglePlayPause() {
	const el = audioVisualizer.value?.audioEl;
	if (el == null) return;
...
// JUICE: 音声にはネイティブのdblclickの代わりになる操作面が無いため、XControlの設定メニューから
// のみ拡大表示を呼び出せるようにする(動画のonDblClickに相当)
...
.audioRoot {
	...
	height: 100%;
	object-fit: contain;
	// JUICE: インライン再生時(inlinePlayable)は中身が全てposition:absoluteの要素(ビジュアライザー・
	// コントロールバー)のみになり、親側に高さの基準が無いと高さが潰れてしまうため、
	// ビジュアライザー本体(XAudioVisualizer .visualizer)と同じ比率をここでも保険として指定しておく
	aspect-ratio: 16 / 9;
}
...
.playIconWrapper {
	...
	display: grid;
	place-items: center;
	// JUICE: 純粋な見た目のオーバーレイなので、下にある要素へのクリック/タップを妨げない
	pointer-events: none;
}

// JUICE: ライトボックスと同じ自作コントロール(XControl)を下端に重ねる。
