...
- Feat: モバイル表示(狭い画面)のウィジェットドロワーで、ウィジェットごとに左寄せ/右寄せを設定できるように。編集モードの各ウィジェットに切り替えボタンを追加し、左寄せに設定したウィジェットがドロワー内で先頭にまとまって表示される
- Feat: 設定の「JUICE」項目にも、ウィジェットの左寄せ/右寄せをまとめて切り替えられる一覧を追加。ウィジェット編集モードの切り替えボタンと同じ設定を参照・変更する
...
- Feat: リレータイムラインのヘッダーオプションメニューに、表示するリレーを1つに絞り込むフィルタを追加(既定はすべてのリレー)
...
- Feat: `notes/relay-timeline`(REST)と`relayTimeline`(stream channel)に`relayId`パラメータを追加。指定したリレー経由のノートのみに絞り込める。フィルタの選択肢用に、受理済みリレーの一覧を返す`juice/relays`を新設
## 2026.7.0-juice+1.0
