...
- Feat: 検索ページに、自分が付けたリアクションでノートを絞り込む専用タブ「リアクション」を追加(JUICE独自)。「何かしらリアクションした投稿」または特定のリアクションを選んでの絞り込みに、キーワード検索・ノート検索タブと同じ高度な検索オプション(OR検索・除外ワード・各種フィルタ)・日時範囲・検索範囲(全体/ローカル/サーバー指定/ユーザー指定)も組み合わせ可能。プライバシー上、自分自身のリアクションのみが対象
