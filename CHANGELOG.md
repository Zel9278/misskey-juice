...
- Fix: 「アカウントを追加」メニューの新規登録項目が、承認式新規登録が有効で招待コード登録の入り口が非表示のサーバー(承認制のみ・招待制ではない)でも常に「アカウントを作成」のまま固定されており、実際には審査が必要なのにその場で作成できるかのような表示になっていた問題を修正(JUICE独自)。ウェルカムページの単一ボタンと同じく「参加を申請する」に出し分けるように変更
