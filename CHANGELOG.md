...
- Enhance: 本家Misskeyには無いJUICE独自の設定・タブ・メニュー項目に「JUICE」バッジを表示するように(ロール設定のemojiRequestLimit、お知らせ作成の投票機能、探索ページの「ユーザーランキング」タブ、タイムラインの「リレー」タブ・リレー絞り込みメニュー、ロゴメニューの「絵文字申請」、本家Aboutページ・ロゴメニューの「JUICEについて」)
