...
### Client
- Feat: 小説エディター(`/novel-editor`)を追加(JUICE独自)。ナビのメニューから開ける
  - 記法(ルビ・傍点などの注記・章タイトル・区切り・改ページ・太字等)に色を付け、ルビ・傍点・装飾・字下げ・章タイトル・区切り・改ページ・――・……をボタンで入力できる
  - 作品ごとの下書き(このブラウザに自動保存、別のタブとも同期)、目次(章ごとの文字数・押すとその位置へ移動)、文字数・原稿用紙換算・選択範囲の文字数・目標文字数
  - 検索・置換、括弧を自動で閉じる・改行したら字下げする、書いている行を画面の真ん中に保つ、集中モード(全画面)、書体・文字サイズ・行の高さの設定
  - 小説ビューワーと同じ見た目のプレビュー(広い画面ではエディターの横に並べ、狭い画面では別ウインドウ。書いている間もそのまま反映される)
  - 投稿前のチェック(ルビの始まりの印・括弧の閉じ忘れ・閉じていない注記・対応していない注記・三点リーダーやダッシュ・！？の後の空白など)を別ウインドウで表示し、押すとその行へ移る。投稿するときにも知らせる
  - .txtの読み込み(端末・ドライブ。文字コードは自動で判定)と保存、本文を.txtにして小説フラグ付きで投稿
- Feat: 絵チャに機能を追加(JUICE独自)
  - 範囲選択・投げ縄選択(選んだ範囲の境目で線を切る)・移動ツール・選んだ線の回転と削除
  - 囲って塗る・バケツ(塗りつぶし)、にじみ筆・ドット筆、線の中だけ塗る
  - ペンと消しゴムの太さを別々に、手のひらツール(表示だけを動かす)、キャンバスの回転(スマホ等は2本指)、PCでの拡大縮小の操作(ホイール・ボタン・Ctrl+＋/－/0)と表示の移動(Shift+ホイール・中ボタン・スペース+ドラッグ)
  - ピクセルアート拡大モード(全体マップにも反映)と、大きく拡大したときの画素の格子。拡大は最大3200%
  - 部屋主がほかの人のレイヤーを消去できる・部屋主も観戦モードにできる(部屋主のまま、満員でも戻れる)
  - チャットで絵文字・カスタム絵文字を使えるように、発言の時刻を表示
  - 部屋を開くときの読み込み中の表示
- Feat: MFMの検索窓(「○○ 検索」)で使う検索エンジンを、JUICE設定で選べるように(JUICE独自)。Google・Yahoo!・Yahoo! JAPAN・Bing・DuckDuckGo・Kagi・Brave Search・Startpage・Ecosia・Perplexity・カスタムのURL
- Enhance: 狭い画面(スマホ等)で、絵チャ・小説エディターのボタンに名前を表示し、ページのヘッダーのボタンは名前付きのメニューにまとめるように
- Enhance: スマホ・タブレットで、ウインドウの大きさを変えるつまみを大きくした
- Fix: 絵チャの終了後の「ドライブに保存」「ノートに投稿」「ダウンロード」が正しく動かない問題を修正
- Fix: 絵チャの終了後に、見学していた人やオンライン表示がレイヤー一覧に残る問題を修正
- Fix: 小説ビューワーの縦書きで、ページが画面の高さを超える問題を修正
- Fix: 小説ビューワーで、縦書きでページを送ったまま横書きにすると本文が表示されない問題を修正
- Fix: 小説ビューワーの横書きで、狭い画面だと章の移動ボタンが見切れる問題を修正
...
### Client
- Feat: 絵チャ(お絵かきチャット)を追加(JUICE独自)。部屋を作って複数人で同じキャンバスに描ける。ナビのメニューと「ゲーム」ページから開ける
  - 公開範囲(フォロワーのみ/ローカル全体)・描ける人数の上限(2〜512人、途中で変更可)・キャンバスの大きさ(プリセットまたは自由に指定、途中で変更可)を部屋主が決められる。満員でも見学者として入れ、チャットは見学者も書ける
  - 線はユーザーごとのレイヤーに分かれ、他人の線を消してしまうことがない。レイヤーごとの表示/非表示、自分のレイヤーを一番上に表示する切り替えに対応
  - ペンの筆圧に対応。ペン・消しゴム・スポイト(I / Altを押しながらクリック)・色・太さ(キャンバスの大きさに合わせて上限が変わる)・濃さ(不透明度)・取り消し(Ctrl+Z)・自分のレイヤーの消去
  - ほかの人のカーソルを丸いアイコン付きで表示。部屋を開いている人(オンライン)を一覧に表示し、閉じた・別のタブへ移った人はオフラインになる
  - 拡大縮小・移動(ホイール/2本指)と、今見ている範囲を示す全体マップ・倍率表示
  - 開催中・終了後どちらでも、全体または選んだ範囲を画像として保存・投稿・ダウンロードできる。形式は無圧縮PNG・WebP(Misskeyのアップロード時の圧縮と同じ)・JPEGから選べる
  - 部屋主は部屋を終了でき、終了後もサーバーに保存しておく(閲覧のみ)か、1時間後に削除するかを選べる
  - スマホや狭いウインドウ(デッキ含む)では、キャンバスを広く取り、レイヤー・チャットを下から出すパネルにした表示になる
- Feat: コントロールパネルのJUICE設定に、絵チャの有効/無効を追加(JUICE独自)
- Feat: ロールポリシーに「絵チャの部屋の作成」「絵チャで作れるキャンバスの大きさの上限」を追加(JUICE独自)
- Feat: 絵チャの部屋そのもの(部屋主)と、部屋のチャットの発言(発言した人)を通報できるように(JUICE独自)。通報した時点の内容を残し、管理画面の通報に表示する
- Feat: モデレーターが問題のある絵チャの部屋を、開催中でも削除できるように(JUICE独自)。削除はモデレーションログに記録される

### Server
- Feat: 絵チャの`draw-rooms/*`エンドポイント(create・show・list・join・leave・update・kick・end・delete・strokes・chat-history)と、ストリーミングの`drawRoom`チャンネルを追加(JUICE独自)。線はバイナリ(base64)で送受信し、描いている途中の線・カーソル・オンライン状態もリアルタイムに配信する
- Feat: 権限`read:draw-rooms`・`write:draw-rooms`、ロールポリシー`canCreateDrawRoom`・`drawRoomMaxCanvasSize`を追加(JUICE独自)
- Feat: 終了して保存しない絵チャの部屋・放置された部屋を片付ける定期処理を追加(JUICE独自)
- Feat: `users/report-abuse`に絵チャの部屋・部屋のチャットの発言を対象にする`drawRoomId`・`drawRoomChatMessageId`を追加し、`admin/abuse-user-reports`で`targetDrawRoom`(通報した時点の写し)を返すように(JUICE独自)
- Feat: モデレーションログの種類に`deleteDrawRoom`を追加(JUICE独自)

## 2026.9.1-juice+3.14

### Client
- Enhance: 小説ビューワーの本文でMFMを解釈しないように変更(JUICE独自)。リンク・メンション・カスタム絵文字等は記法のまま表示する。ルビ(MFM・青空文庫記法・pixivの`[[rb:]]`)と、`**太字**`・`*斜体*`・`~~打ち消し線~~`のみ反映する
- Feat: 小説ビューワーでpixiv小説の記法に対応(JUICE独自)。`[newpage]`で改ページし、横書きでは1ページずつ表示して前後のページへのボタンで移動できる。`[chapter:タイトル]`は目次の章名として使う
- Feat: 小説ビューワーに本文の文字数と、読み終わるまでのおおよその時間を表示(JUICE独自)
- Enhance: 小説ビューワーの段落の自動字下げで、「」等の始め括弧や、中黒・ダッシュ・傍点・●○等の記号で始まる行は字下げしないように(JUICE独自)
- Fix: 小説ビューワーの縦書きで、…や―、半角の括弧等が横向きのまま表示される問題を修正(JUICE独自)
- Fix: 小説ビューワーの縦書きで、ウインドウの大きさを変えるとページの区切りが追従しなかったり、読んでいた位置が大きくずれたりする問題を修正(JUICE独自)
- Fix: 小説ビューワーの縦書きで、ページの端に隣のページのルビの切れ端が見えることがある問題を修正(JUICE独自)
- Fix: 小説ビューワーの縦書きで、ブラウザのページ内検索等で本文の枠がスクロールされると、左右のページの内容がずれる問題を修正(JUICE独自)
- Enhance: 小説ビューワーで、EUC-JPの.txtファイルも自動判別して読めるように(JUICE独自)。あわせて、ルビの基底の始まりの印として｜のほかに＊・*・|も受け付けるように
- Feat: 小説ビューワーで青空文庫記法の傍点(［＃「対象」に傍点］、［＃傍点］…［＃傍点終わり］。白ゴマ・丸・白丸も)に対応(JUICE独自)
- Feat: 小説ビューワーに全画面表示を追加(JUICE独自)。本文を画面いっぱいに広げて読め、全画面のままでも目次・表示設定・縦書き/横書きの切り替えができる

## 2026.9.1-juice+3.13

### Client
- Feat: 投稿に「小説」フラグを付けられるように追加(JUICE独自)。投稿フォームのAI生成トグルの横にトグルを置き、投稿後もノートのメニューから切り替えられる。フラグ付きの投稿には本のバッジを表示する
- Feat: 小説フラグ付きの投稿を読むための小説ビューワーを追加(JUICE独自)。横書きと縦書き(文庫本風のページめくり、広い画面では見開き表示)を切り替えられ、章ごとの目次・ルビ・青空文庫記法・段落の字下げ・文字サイズ/書体/背景テーマ・しおり(読んでいたページの記憶)に対応。添付した.txtファイルを本文として読み込めるため、文字数上限を超える長編も投稿できる
- Feat: ドライブの.txtファイルに小説フラグを付けられるように追加(JUICE独自)。フラグ付きのファイルを投稿に添付すると、投稿にも自動で小説フラグが付く
- Feat: 通常のタイムライン(ホーム/ローカル/ソーシャル/グローバル)のメニューに「小説のみ表示」を追加(JUICE独自)。小説フラグ付きの投稿は、添付ファイルが無くてもメディアタイムラインに表示される
- Enhance: JUICEについてページの機能一覧に、小説フラグ・小説ビューワーの項目を追加(JUICE独自)
- Fix: JUICEについてページの機能一覧で、連携ログインの項目の長い英字列がカードの外へはみ出していた問題を修正(JUICE独自)

### Server
- Feat: ノートとドライブのファイルに小説フラグ(`isNovel`)を追加(JUICE独自)。投稿後にフラグだけを切り替える`notes/juice/update-novel`エンドポイント、タイムライン系エンドポイント・ストリーミングの`onlyNovel`オプションを追加
- Feat: 小説フラグを解釈できない非JUICE実装向けに、連合時に小説であることをCWとして送るフォールバック設定をコントロールパネルのJUICE設定に追加(JUICE独自、既定は無効)

## 2026.9.1-juice+3.12

### General
- 本家Misskey 2026.9.1の変更をすべて取り込み(SQLの取り扱い強化・招待コード使用時の競合対策・WebSocketの購読数上限など、セキュリティ修正を含む)。詳細は下記の「2026.9.1」を参照

### Client
- Feat: 装飾的なMFMを含む投稿を自動で「ローカルのみ」にする設定を追加(JUICE独自)。設定の「JUICE」ページで「標準マークダウン系(太字・斜体・取り消し線・コード)」と「MFM独自の装飾系(center・small・quote・search・数式・$[]関数)」を個別にON/OFFでき、本文・CWのどちらかに該当構文が含まれていれば発火する。ダイレクトメッセージ・リモートユーザーへの返信は対象外

### Server
- Feat: 上記の自動ローカル化設定を更新する`i/juice/update-auto-local-only-for-mfm`エンドポイントを追加(JUICE独自)
- Fix: 新規登録時のユーザー名の空き状況チェックが、禁止ワード(ユーザーの名前)を考慮していなかった問題を修正(JUICE独自)。入力中は「使えます」と表示されるのに送信すると弾かれることがあった
- Enhance: 3.11で追加したJPEG XL・HEIC/HEIFの表示対応(専用WASMデコーダ)を廃止(JUICE独自)。画像処理失敗時に404へフォールバックする修正は維持

## 2026.9.0-juice+3.11

### Client
- Enhance: MIDIプレイヤーの拡大表示を画像/動画と同じライトボックスに統合(JUICE独自)。拡大時はインライン側と同じ再生状態(再生中か・再生位置・音量)を引き継ぐ。ライトボックスの「…」メニューにMIDI設定(ビジュアライザー有効/無効・ロール速度・同時発音数)も統合
- Fix: MIDIの鍵盤ハイライトが、サステインペダルによって実際の発音より長く点灯し続ける問題を修正(JUICE独自)。鍵を離した時点で即座に消灯するように
- Fix: iPhone/iPadでは端末のサイレントスイッチがオンだとMIDIプレイヤーの音が鳴らない既知の挙動について、該当環境で警告を表示するように追加(JUICE独自)
- Enhance: MIDIファイルのアイコンに、JUICE独自機能であることを示す小さいバッジを追加(JUICE独自)

### Server
- Enhance: `/proxy/`でJPEG XL・HEIC/HEIFファイルを表示できるように対応(JUICE独自)。専用のWASMデコーダで変換してから既存のリサイズ/WebP変換パイプラインへ渡すようにし、これらの形式のリモート絵文字・アバター等が表示できなかった問題を解消
- Fix: ファイルのmime誤検出等に起因する想定外の例外が、生の500エラーとして返っていた問題を修正(JUICE独自)。画像処理に失敗した場合は404へ適切にフォールバックするように

## 2026.9.0-juice+3.10

### Client
- Feat: MIDIプレイヤーにピアノロールビジュアライザー(縦スクロール、テンポ変化に強いtick基準の再生位置表示)を追加(JUICE独自)。有効/無効・ロール速度・最大同時発音数を設定の「JUICE」ページとプレイヤー内メニューの両方から変更できる
- Fix: 黒MIDI等、ノートイベント数が極端に多いファイルの再生中に音が途切れる・フリーズする問題を修正(JUICE独自)
- Fix: テンポ変化のある曲でMIDIファイルをシークすると再生位置がずれる問題を修正(JUICE独自)
- Enhance: リレータイムラインのブースト元表示バッジを撤去(JUICE独自)
- Fix: JUICE単独のバージョン更新(本家Misskeyのバージョン表記が変わらない場合)ではアップデート通知ダイアログが表示されなかった問題を修正(JUICE独自)。ダイアログの内容もMisskey/Juiceそれぞれのバージョンを表示する形に変更し、変更履歴へのリンク先をJUICEのchangelogページに変更
- Enhance: JUICE独自機能であることを示すアイコン・バッジを整理(JUICE独自)。タブ・投稿フォームの雫アイコンを専用のジュースグラスアイコンに、AI生成物関連のアイコンをti-aiに統一
- Enhance: about-juiceページの機能一覧に、連携ログイン・MIDIプレイヤー・通報のカテゴリ分類・モデレーション向け通知の項目を追加(JUICE独自)

### Server
- Enhance: リレータイムラインのブースト元記録・連合機能を撤去(JUICE独自)。`relayAnnouncerId`カラムを削除するマイグレーションを追加

## 2026.9.0-juice+3.9

### Client
- Feat: ノート添付のMIDIファイルを再生できる軽量プレイヤーを追加(JUICE独自)。外部サウンドフォント等を使わずWeb Audio APIのオシレーター合成のみで動作し、GM音色ファミリーごとの合成レシピ・コントロールチェンジ・ピッチベンドに対応。128鍵の鍵盤ビジュアライザー(トラックごとの色分け)・音量/シークバー・BPMとノート再生数のライブ表示・AudioWorkletによるピークリミッター・Media Session API連携(ロック画面/通知領域からの再生操作)を備える
- Enhance: リレータイムラインで、ブーストによって転送されたノートの投稿元を分かりやすく表示するように改善(JUICE独自)。従来はリレーに登録されていないサーバーの投稿が流れてきたように見えてしまい分かりにくかった
- Fix: リレーサーバーを削除して再登録すると、以前そのリレーだけに絞り込んでいたユーザーのリレータイムラインが理由もわからず空のままになる問題を修正(JUICE独自)。存在しなくなったリレーIDを絞り込み設定から自動的に取り除くように

### Server
- Feat: MIDIプレイヤーの再生を許可する最大ファイルサイズを管理者設定で変更できるように追加(JUICE独自、既定500KB)。黒MIDI等、ノートイベント数が極端に多いファイルの解析・再生によるブラウザのフリーズを防ぐための安全装置
- Enhance: リレータイムラインで、ブーストによって転送されたノートの投稿元(誰がブーストしたか)を記録・連合するように追加(JUICE独自)
- Fix: リレーの削除・承認・却下操作で、承認済みリレー一覧の内部キャッシュ(最大10分)が即座に無効化されていなかった問題を修正(JUICE独自)。削除直後に同じホストを再登録すると、存在しないリレーIDへの書き込みで外部キー違反が発生し、inboxの配送処理が失敗することがあった

## 2026.9.0-juice+3.8

### Client
- Feat: Discord/Google/GitHub/GitLab/Microsoftのアカウントを使った連携ログイン機能を追加(JUICE独自)。設定の「JUICE」ページでアカウントの連携・解除、サインイン手段として使うかどうかの切り替えができ、サインイン画面にも有効なプロバイダごとのボタンを表示する
- Feat: コントロールパネルのJUICE設定に、連携ログインの各プロバイダの有効化・クライアントID・クライアントシークレットの入力欄と、共通のリダイレクトURI・各プロバイダの開発者ポータルでの設定手順の案内を追加(JUICE独自)
- Enhance: 設定の「JUICE」ページを「アカウント」「タイムライン・表示」「申請フォーム」の3カテゴリに整理(JUICE独自)。従来は多数の設定項目がフラットに並んでおり見通しが悪かったための改善

### Server
- Feat: Discord/Google/GitHub/GitLab/Microsoftのアカウントを使った連携ログイン(OAuth2認可コードフロー)をサーバー側に実装(JUICE独自)。連携ログインをサインイン手段として使うには、ユーザー自身が2段階認証を有効にしたうえで個別にオプトインする必要があるようサーバー側で強制しており、連携先アカウントの乗っ取りだけでサインインされてしまうことを防ぐ(本家Misskeyがこの機能を過去に廃止した理由への対策)

## 2026.9.0-juice+3.7

### Client
- Feat: 絵文字申請・アバターデコレーション申請フォームで、カテゴリ・タグ・ライセンス・説明といった項目を管理者設定で個別に必須化できるように追加(JUICE独自、既定は任意)。必須項目のラベルには「(必須)」を表示し、未入力の場合は申請ボタンを無効化する
- Enhance: 上記フォームで、複数件まとめて申請しようとする際にどのカードのどの項目が未入力か分かりやすいよう、該当するカードのヘッダーに警告バッジを表示するように追加(JUICE独自)
- Fix: 絵文字・アバターデコレーション申請で、申請をキャンセル・送信失敗した場合にDriveへアップロード済みの画像が削除されずに残ってしまう問題を修正(JUICE独自)
- Feat: Driveの複数選択モードに、選択した画像をまとめて削除できるボタンを追加(JUICE独自)
- Feat: デッキUIの「タイムライン」カラムに、通常のタイムラインページにあった「ホームTLのローカルユーザー限定表示」「表示言語での絞り込み」の切り替えを追加(JUICE独自)。従来はデッキだけこれらの設定ができなかった

### Server
- Enhance: AI生成物フラグ(`isAIGenerated`)を解釈できない非JUICEインスタンス向けのCWフォールバックを改良(JUICE独自)。既にCWが設定されている場合は「フォールバック文言 | 元のCW」の形で合成するように変更(従来はCW未設定の投稿のみが対象で、CW自体は上書きしていなかった)。また判定条件をノート本体の`isAIGenerated`だけでなく、添付ファイルのいずれか1件でもAI生成フラグが立っていれば対象にするよう拡張。JUICE間の連合では元のCWを`_juice_originalCw`として別途送出し、復元できるようにした
- Feat: 絵文字申請・アバターデコレーション申請フォームの必須項目設定(カテゴリ・タグ・ライセンス・説明)を、管理者設定として追加(JUICE独自、既定は任意)
- Feat: Drive画像の複数選択削除エンドポイント(`drive/files/delete-bulk`)を追加(JUICE独自)
- Fix: 表示言語での絞り込み(`filteredLanguages`)がリレータイムラインにだけ適用されていなかった問題を修正(JUICE独自)。REST・リアルタイムストリームの両方に対応
- Enhance: 表示言語での絞り込み・投稿言語のActivityPub連合(`contentMap`)を、Mastodon/Pleroma/Akkoma等の地域無し言語コード(例: `en`)と互換性を持つように変更(JUICE独自)。フィルタ側は主言語サブタグ単位でマッチするようにし(中国語はスクリプトの違いを区別するため完全一致を維持)、連合へ送出するcontentMapの言語タグもMastodon互換の形へ正規化。JUICE間の連合ではリージョン情報を保持した元の言語タグを`_juice_lang`として別途送出する

## 2026.9.0-juice+3.6

### Client
- Feat: 絵文字申請・アバターデコレーション申請フォームに、審査待ち件数の上限とは別の「1日あたりの送信回数」の残り回数を表示するように追加(JUICE独自)。上限に達した場合のエラーダイアログも、汎用文言ではなく機能固有の案内文に差し替え
- Feat: 通報の新着通知を、通常の通知(🔔の通知一覧)にも残るように追加(JUICE独自)。従来はコントロールパネルを開いている間のリアルタイムトーストのみで、オフラインだったりリロードした後は遡って確認する手段が無かった
- Fix: メディアタイムラインで、画像付き投稿への純粋なリノート(引用ではないリノート)が表示されない問題を修正(JUICE独自)
- Enhance: 絵文字申請・アバターデコレーション申請フォームの「1日あたりの送信回数」表示に、次に枠が回復する日時(ETA)を追加(JUICE独自)。この制限が暦日リセットではなく直近24時間のスライディングウィンドウであることに合わせ、案内文言も修正
- Enhance: 上記フォームのインフォカードを整理(JUICE独自)。「審査待ち件数」「1日あたりの残り送信回数」「今回選択中の件数」「複数選択のヒント」の最大4枚に積み重なっていた表示を、状態カード1枚+ヒントカード1枚(未選択時のみ)の最大2枚に集約

### Server
- Feat: 絵文字申請・アバターデコレーション申請の1日あたりの送信回数上限(審査待ち件数の上限=`emojiRequestLimit`等とは別のAPI呼び出し頻度の制限)を、ロールポリシーとして設定可能に追加(JUICE独自、既定値5)
- Feat: AI生成物フラグ(`isAIGenerated`)を解釈できない非JUICEインスタンス向けに、CWが未設定のAI生成ノートに限りActivityPubのsummary(CW相当)へフォールバック文言を合成して送出するオプションを追加(JUICE独自、既定無効。ローカル・JUICE間の表示は目印プロパティによりバッジ表示のまま維持)
- Fix: 通報の新着通知が、ロール割り当てを一切持たないブートストラップ直後のrootに届かない問題を修正(JUICE独自)
- Fix: メディアタイムライン(`withFiles`指定時)の対象判定が投稿自体のファイルしか見ておらず、リノート元のファイルを考慮していなかった問題を修正。DB直接クエリ・fanoutキャッシュ・リアルタイムstreamの3経路すべてで発生していた(JUICE独自)
- Fix: 絵文字申請・アバターデコレーション申請の「1日あたりの送信回数」表示が、本番以外の環境では常にnull(非表示)になっていた問題を修正(JUICE独自)。実際のレート制限適用は引き続き本番環境限定のまま

## 2026.9.0-juice+3.5

### Client
- Fix: リレー・メディアタイムライン表示中にリロードすると、意図せずホームタイムラインへ戻されてしまう問題を修正(JUICE独自)
- Feat: 設定の「JUICE」ページの「表示するタブ」に、タイムラインのタブバーに出るリスト・アンテナ・チャンネルの切り替えショートカットアイコンも、個別に非表示にできる項目を追加(JUICE独自)
- Feat: 地震情報ウィジェットを追加(JUICE独自)。P2P地震情報(気象庁の情報を配信する無料コミュニティAPI)から直近の地震情報(震度・震源地・マグニチュード・発生時刻)を一覧表示する。サイドバー・デッキの「ウィジェット」カラムどちらにも設置可能
- Feat: 通報時にカテゴリを選択できるように追加。ノート・チャットメッセージを通報した場合は対象を構造化して紐付け、モデレーター向けの通報詳細画面でプレビュー表示する(JUICE独自)。あわせて通報一覧のカテゴリ絞り込み・ツールメニュー/専用画面(`/abuses-manager`)からのクイックアクセスも追加
- Feat: デッキの「タイムライン」カラムの種別選択に、リレータイムライン・メディアタイムラインを追加(JUICE独自)。カラムごとに配信元リレー・対象範囲を個別設定できる
- Feat: 設定の「JUICE」ページの「表示するタブ」で、タブの並び順もドラッグで変更できるように追加(JUICE独自)。並び順はタイムラインページのタブバーにも反映される
- Enhance: メディアタイムラインのインラインカルーセルのスワイプ判定を距離・速度ベースに変更して緩和。マウス操作時のみ表示する左右送りボタンを追加し、切り替えアニメーションを高速化(JUICE独自)
- Feat: 管理者/モデレーター限定の特権として、ユーザーを凍結する際に操作者自身のアカウントでブロックしたうえで凍結する「ブロックしてから凍結」を追加(JUICE独自)。コントロールパネルのユーザー詳細画面・ユーザーメニューから利用可能
- Feat: 絵文字申請・アバターデコレーション申請のフォームに、現在の審査待ち件数・残り申請可能数・今回選択中の件数・1回にまとめて申請できる上限を表示するように追加(JUICE独自)。上限に達している場合はファイル選択を無効化し、1回の操作で上限を超える枚数を選んだ場合はトーストで通知する

### Server
- Fix: リレー経由で転送された投稿のLD-Signature検証中に発生するJSON-LDエラーで、inboxキューのジョブがクラッシュして無駄なリトライを繰り返していた問題を修正
- Feat: 作成から一定時間が経過していないアカウント(ローカル・リモート問わず)からのフォローを、フォロー先が承認制フォローを有効にしていなくても承認が必要なフォローリクエストとして扱える設定を追加(JUICE独自、既定は無効。登録直後の荒らし・スパムアカウントによる一括フォロー対策)。設定の「JUICE」ページから有効化・しきい値(分/時間/日で指定可能)を変更できる
- Feat: 通報カテゴリを管理者側で追加・編集・並び替えできる設定を追加(JUICE独自)。ノート/チャットメッセージを通報した場合、対象への構造化参照をDBに保存し、モデレーター向けAPIでプレビュー用データを返す(チャットメッセージの内容を、通報を受けた場合のみ運営が確認できる初めての経路)
- Feat: メールアドレスの別名(Gmailのドット無視・+タグ)を使った複数アカウント登録を、新規登録・メールアドレス変更時の重複チェックで検出してブロックできる設定を追加(JUICE独自、既定は無効)。誤検知リスクの異なるドット無視と+タグを個別にON/OFFできる

## 2026.9.0-juice+3.4

### Client
- Feat: 添付ファイル付きノートだけをグリッド/カルーセル表示で集める「メディアタイムライン」を追加(JUICE独自、PixelFed風UI)。コントロールパネルのJUICE設定で有効化した場合のみタブとして表示される(既定は無効)。複数枚添付時はスワイプ送りのカルーセル表示になり、動画はその場でインライン再生できる専用の操作バー(ループ再生・再生速度・ピクチャインピクチャ・拡大表示)を備える
- Feat: 投稿フォームに「メディアタイムラインに表示しない」を追加(JUICE独自、メディアタイムライン機能が有効なサーバーでのみ表示)。プロフィールのファイルタブ等、他のファイル絞り込み表示には影響しない
- Feat: メディアタイムラインでの音声再生を、動画と同様に拡大表示を経由せずその場でインライン再生できるように追加(JUICE独自)。ライトボックスと同じビジュアライザー(投稿者アバター+波形)とコントロールバーを再利用する
- Enhance: ライトボックス・メディアタイムラインのインライン再生全体で音量設定を共有するように変更(JUICE独自、端末ローカルに保存。一度調整すれば、以後どの動画/音声を開いても同じ音量になる)
- Feat: 設定の「JUICE」ページに、タイムラインページのタブバーに出すベーシックタイムライン(ホーム/ローカル/ソーシャル/グローバル)・リレー・メディアタイムラインを個別に非表示にできる項目を追加(JUICE独自)。サーバー側で無効化されているタブには影響しない

### Server
- Feat: メディアタイムラインの表示対象から投稿ごとに除外できる`hideFromMediaTimeline`フラグを追加(JUICE独自)。メディアタイムライン(withFiles指定のタイムライン)からのみ除外され、通常のタイムライン・プロフィールのファイルタブ・検索には影響しない

## 2026.9.0-juice+3.3

### Client
- Fix: 絵文字申請・承認式登録申請等の未対応バナーが、対応済みになってもコントロールパネルを開いている間は消えなかった問題を修正(JUICE独自)
- Fix: スマートフォンでのナビゲーションバー・絵文字パレット・ウィジェットの並べ替えが、タッチ操作(特に長押し)では正しく機能しなかった問題を修正。Pointer Events採用に書き換え、実機でも並べ替えできるように
- Fix: 新規登録後のプロフィールセットアップダイアログで、横長など縦方向が狭い画面において、アイコン変更・保存ボタン等がフッターの「戻る/続ける」ボタンと重なって表示される問題を修正

## 2026.9.0-juice+3.2

### Client
- Feat: リレータイムラインに、各ノートの配送元リレーのホストを表示するように追加(JUICE独自)
- Feat: ウィジェットとしてもお気に入り一覧を表示できるように追加(JUICE独自、既存のデッキ専用カラムと同じ内容)。サイドバーやデッキUIのウィジェットカラムに追加可能
- Fix: リレータイムラインタブのJUICE独自タブであることを示す小さいマークの色が、テーマ依存の汎用色(--MI_THEME-success)のままで、投稿フォームのAI生成物ボタン等で使っているJUICEブランドカラーと揃っていなかった問題を修正。あわせて、単色の丸ドットだと通知の未読バッジと紛らわしいため、投稿フォームのAI生成物ボタンも含めて小さい雫アイコンに統一
- Enhance: 「削除して編集」で開いた投稿フォームを誤って閉じてしまわないよう、閉じる前に確認ダイアログを挟むように追加(JUICE独自)
- Fix: 「削除して編集」の内容が、何も編集せず閉じた場合に下書きとして保持されず消えてしまう問題を修正。閉じた後、同じコンテキスト(新規投稿・返信・引用)で改めて投稿フォームを開くと内容が復元される
- Feat: ウェルカムページ・「アカウントを追加」メニューの「招待コードで登録」ボタン、およびウェルカムページの「他のサーバーを探す」ボタンを、それぞれ管理者設定で非表示にできるように追加(JUICE独自、既定は両方とも表示)。招待コード登録のボタンを非表示にしても、招待コード自体による登録機能(承認式登録のバイパスを含む)は無効にならず、招待した相手に「?invite」を付けたURLを個別に共有すればボタン無しでも招待コード登録フォームを直接開ける
- Fix: 絵文字申請・アバターデコレーション申請・承認式登録申請・お問い合わせの新着通知(バナー・リアルタイムトースト)が、モデレーターでなければコントロールパネル(/admin)自体に到達できず、対応するロールポリシー(canApproveEmojiRequests等)だけを個別に付与された非モデレーターユーザーには一切届いていなかった問題を修正(JUICE独自)。アプリ全体で常時、該当ユーザーへリアルタイム通知するように変更
- Fix: アカウントメニューの「ツール」内にある絵文字申請・アバターデコレーション申請の承認画面へのリンクが、同じメニュー内にある一般ユーザー向けの申請メニュー項目(「絵文字申請」「アバターデコレーション申請」)と全く同じ表示名でダブっていた問題を修正(JUICE独自)。ツール側はそれぞれ「絵文字申請を確認」「アバターデコレーション申請を確認」に変更
- Fix: 絵文字申請フォームのリアクション見た目プレビューで、実際には正しく選択・アップロードできている画像が、ダミー画像(読み込み失敗時の代替表示)になってしまう問題を修正(JUICE独自)。ドライブから選択した場合はreactionEmojisのキー形式の食い違いが、PCから直接ファイルを選択した場合はblob: URL(ブラウザタブ内限定で、サーバー側からは絶対に取得できない一時URL)を画像プロキシ経由で表示しようとしていたことがそれぞれ原因だった
- Fix: ジョブキューウィジェットの「音を鳴らす」設定の初期値がオンになっており、意図せず音が鳴ってしまうことがあった問題を修正。初期値をオフに変更(JUICE独自)
- Fix: 絵文字申請・アバターデコレーション申請フォームで、まとめて複数件選択していない(1件だけの)場合、選び直すための削除ボタンが表示されず、選択をやり直せなかった問題を修正(JUICE独自)
- Feat: 絵文字申請・アバターデコレーション申請について、審査待ちの間であれば申請者自身がキャンセル(取り下げ)できるように追加(JUICE独自)。キャンセルした申請は「結果」タブから「キャンセル済み」として確認できる
- Fix: アカウント切り替えメニュー・設定の「アカウント」ページ、どちらの「アカウントを追加」の新規登録項目も、承認式新規登録が有効で招待コード登録の入り口が非表示のサーバー(承認制のみ・招待制ではない)では常に「アカウントを作成」のまま固定されており、実際には審査が必要なのにその場で作成できるかのような表示になっていた問題を修正(JUICE独自)。ウェルカムページの単一ボタンと同じく「参加を申請する」に出し分けるように変更。設定の「アカウント」ページ側は従来この判定自体を一切行っておらず常に固定2択だったため、アカウント切り替えメニュー側と判定ロジックを共有するようにした
- Feat: 絵文字申請・アバターデコレーション申請・承認式登録申請・お問い合わせの新着通知を、通常の通知(🔔の通知一覧)にも残るように追加(JUICE独自)。従来はアプリを開いている間のリアルタイムトースト・バナーのみで、オフラインだったりリロードした後は遡って確認する手段が無かった
- Feat: 標準テーマにJuice Orange(ライト/ダーク)を追加し、標準の初期テーマとして設定(JUICE独自)
- Fix: 通知の受信設定(/settings/notifications)・通知ウィジェットの種別選択に、JUICE独自の通知種別であることを示すJUICEバッジが付いていなかった問題を修正

### Server
- Fix: リアクション相乗りでリモートの絵文字を使った場合、配信先が本家Misskey等(JUICE/tempura以外)だと絵文字リアクションとして認識されずハートにフォールバックしてしまう問題を修正。受信側の絵文字判定が`@実ホスト名`付きの文字列を認識できないことが原因だったため、相乗り時のcontent/`_misskey_reaction`はベアな`:name:`形式で送信し、実際の絵文字画像はtagのみで伝えるように変更(前回3.1で対応したtag自体の付与漏れは修正済みだったが、この形式の問題が残っていた)
- Fix: タイムラインに表示する言語のフィルターが、ハイブリッドタイムライン(REST)およびホーム・ローカル・ハイブリッド・グローバルタイムラインのリアルタイムストリーミングには適用されておらず、フォローしているユーザーのリロードなしのリアルタイム投稿やハイブリッド/グローバルタイムラインの投稿が絞り込まれない問題を修正
- Feat: 表示言語の絞り込みが有効な場合でも自分自身の投稿を常に表示するかどうかを設定できるように追加(JUICE独自、既定はオン=常に表示。オフにすると自分の投稿も他のユーザーと同様に言語で絞り込まれる。グローバルタイムラインには元々この特例が無く、この設定の影響も受けない)
- Enhance: 表示言語の絞り込みが有効(1つ以上選択)なとき、言語が指定されていない投稿もフィルタ対象(非表示)にするように変更(JUICE独自)。従来は絞り込みの有無に関わらず常に表示されていた。絞り込みを1つも選択していない場合の挙動は変更なし
- Fix: タイムラインの「…」メニューから表示言語の絞り込みを変更しても、既に表示中のタイムラインに即座に反映されず、再読み込みするまで古い絞り込み結果のまま残ってしまう問題を修正
- Fix: 絵文字申請・アバターデコレーション申請・承認式登録申請・お問い合わせの新着通知が、モデレーター(isModerator/isAdministrator)にしか送られておらず、対応するロールポリシー(canApproveEmojiRequests等)だけを個別に付与された非モデレーターユーザーには送られていなかった問題を修正(JUICE独自)
- Feat: 絵文字申請・アバターデコレーション申請を審査待ちの間に申請者自身がキャンセルできるエンドポイントを追加(JUICE独自)。それぞれのstatusに、モデレーターによる却下(rejected)とは別の値としてcancelledを追加した
- Feat: 絵文字申請・アバターデコレーション申請・承認式登録申請・お問い合わせの新着通知を、通常の通知(notificationTypesにnewEmojiRequest等を追加)にも残すように追加(JUICE独自)。申請者はミュートフィルタの影響を受けない専用フィールド(requesterId/applicantId)で持たせており、モデレーターが申請者を別件でミュートしていても通知が届く

## 2026.9.0-juice+3.1

### Client
- Enhance: 絵文字クリック時の「情報」メニューを、ローカルのカスタム絵文字だけでなくリモートサーバーのカスタム絵文字(ノート本文・CW・プロフィール等に埋め込まれたもの)でも利用可能に。ライセンス等の詳細情報と絵文字の提供元ホストを確認できる
- Fix: お気に入りデッキカラムでノートの間隔が広すぎた問題を修正、他のデッキカラムと同じ表示に統一
- Fix: お気に入りデッキカラムを表示中に同一クライアント内でノートをお気に入りに追加/解除しても、リアルタイムに反映されない問題を修正
- Enhance: ジョブキューウィジェットの「鳴らす音」設定を、音の選択肢に「無し」を含める方式から、有効/無効の切り替え(音を鳴らす)と鳴らす音の選択を分離する方式に変更(JUICE独自)。無効化するたびに選んでいた音を忘れてしまう問題を解消
- Enhance: リアクション相乗りでリモートの絵文字を使う際の著作権に関する注意書きを、管理画面の設定項目(有効時のみ)と「JUICEについて」ページの機能一覧(有効時のみ)に追加(JUICE独自)

### Server
- Fix: リアクション相乗りでリモートの絵文字を使った場合、ActivityPub配信時に絵文字画像の情報(tag)が付与されず、配信先でハートにフォールバックしてしまう問題を修正
- Feat: お知らせに付けられるリアクションの種類数の上限をロールポリシーで設定できるように追加(JUICE独自、既定20)
...
- Enhance: 「JUICEについて」ページの機能一覧に、アバターデコレーション申請とロール単位の承認権限委譲の説明を追加。項目が増えて長くなってきたため折りたたみ表示に変更
...
### Client
- Fix: 管理画面・設定のサイドバーで、コントロールパネル「絵文字申請」「承認待ちユーザー」・Webhookイベント「絵文字申請があったとき」「承認式登録の申請があったとき」等、JUICE独自の項目にJUICEバッジが付いていなかったのを修正(サイドバー共通コンポーネントにバッジ表示の仕組みが無かったため)
- Feat: 絵文字申請と同様に、一般ユーザーがアバターデコレーションを申請できる新規ページ「アバターデコレーション申請」を追加(JUICE独自、設定の「JUICE」項目からリンク)。申請フォーム・自分の申請一覧では、アップロードした画像を自分のアバターに重ねたプレビューを確認できる。コントロールパネルに承認待ち一覧・承認・却下画面も追加

### Server
- Feat: 絵文字申請と同じ仕組みで、アバターデコレーション申請機能を追加(JUICE独自)。承認時はシステム所有の複製を作成してから登録する方式(絵文字申請と同様、審査後にファイル削除しても影響を受けない)。JUICE設定で機能の有効・無効を切り替え可能、ロールポリシーに同時に出せる申請数の上限を追加、管理者向け通知(バナー・リアルタイムトースト・Webhook)・審査結果メール・Discord Webhook自動整形にも対応
...
### Client
- Fix: リレータイムラインのタブを、`gtlAvailable`ポリシーが無効な場合は表示しないように(サーバー側の制限追加に追従)
- Fix: 承認待ちアカウントでサインインした際、汎用エラーではなく承認待ちである旨のダイアログを表示するように
- Feat: コントロールパネルに JUICE 独自機能設定用の新セクション「JUICE」を追加
- Feat: 承認式新規登録を追加。有効時は新規登録に理由の入力が必須になり、Moderator/adminが承認するまでサインインできない
- Feat: コントロールパネルに承認待ちユーザーの一覧・承認・却下画面を追加
- Feat: 招待コードが必須のサーバーで承認式新規登録も有効な場合、新規登録画面に「招待コードで登録」「参加を申請する」の2つのボタンを表示するように
- Feat: プロフィールメニューの「アカウントを追加」からも、承認式新規登録が有効な場合は同様に「招待コードで登録」「参加を申請する」を分けて表示するように
- Feat: 承認式新規登録が有効なサーバーでは、訪問者向けトップページ・登録前のルール確認画面・新規登録フォームに「このサーバーは承認式です」という案内を表示するように
- Feat: メールアドレスの入力が必須なサーバーでは、新規登録フォームでメールの受信言語を選択できるように。選んだ言語は以降そのアカウント宛のシステムメール(パスワードリセット・ログイン通知・承認/却下通知等)すべてに使われる
- Feat: コントロールパネルの JUICE 設定に、メールの受信言語を選択していないユーザー向けの既定言語を設定する項目を追加
- Feat: 設定画面に「JUICE」項目を追加し、登録後でもメールの受信言語を変更できるように
- Feat: 投稿にAI生成物であることを示す「AI Generated」フラグを追加。投稿画面のツールバーからON/OFFを切り替えられ、添付したDriveファイルにも連動する
- Feat: ノートに「AI Generated」バッジを表示。リノート・引用ノートでも元ノートのフラグが表示される
- Feat: ノートメニューから自分の投稿のAI生成物フラグを投稿後でも変更できるように
- Feat: Drive画面に「AI生成物のみ表示」フィルタを追加
- Feat: ドライブのファイルメニューから、アップロード後でもAI生成物フラグを付け外しできるように(これまでは投稿フォームの添付時のみ変更可能だった)
- Feat: ノートに添付された画像のうちAI生成物フラグが付いているものに、GIF/ALT/センシティブと同じインジケーターでバッジを表示するように(ノート全体のAI Generatedバッジとは独立して、ファイル単位で表示される)
- Feat: 投稿フォームのアップロード待ちファイルのメニューに「AI生成」スイッチを追加。センシティブと同様、アップロード前にファイル単位でAI生成物フラグを設定できるように
...
- Feat: メールアドレスを収集しない(メール必須ではない)サーバーでも、承認式新規登録の申請時に確認コードを発行するように。登録完了時にコードがこの端末へ自動保存され、新設の「登録審査状況の確認」ページ(`/signup-check`)からいつでも審査状況(審査待ち・承認・却下)を確認できる。1端末から複数アカウント分申請した場合もそれぞれ個別に追跡できる。設定の「JUICE」項目からもこのページへのリンクを追加(ログイン済みでもアクセス可能)。登録完了時の案内は確認コードをコピーボタン付きの専用ダイアログで表示するように。未ログイン時のトップ画面からは、画面右側からスライドインするパネル(幅の狭い画面では全画面表示)で開くように
- Feat: お知らせに投票機能を追加(JUICE独自実装)。コントロールパネルの「お知らせ」作成画面で、新規作成時のみ選択肢・複数選択可否・期限を設定した投票を追加できる(作成後の編集は不可)。お知らせページ・お知らせ一覧のリアクション欄の上に表示され、一般ユーザーが投票できる
- Feat: LaTeX(数式)表示機能を復旧(JUICE独自実装)。本家Misskeyが2023年に削除したKaTeXによる数式レンダリングを、`\href`等の危険なコマンドを無効化する設定(`trust: false`)込みで復旧した。コントロールパネルの JUICE 設定から無効化すると、これまで通り数式ソースがそのままコードとして表示される(既定は有効)
...
- Fix: 承認式登録の審査状況確認パネルの「サインインする」ボタンが、パネル自体より低いz-index優先度でサインインダイアログを開いてしまい、パネルの裏に隠れて見えなくなる問題を修正
- Fix: 承認式登録の審査状況確認画面で、承認済みエントリの「サインインする」ボタンが、ブラウザが別アカウントで既にログイン中だと無反応になる問題を修正(ログイン状態に関わらず常にサインインダイアログを開くように変更)
- Enhance: 本家Misskeyには無いJUICE独自の設定・タブ・メニュー項目に「JUICE」バッジを表示するように(ロール設定のemojiRequestLimit、お知らせ作成の投票機能、探索ページの「ユーザーランキング」タブ、タイムラインの「リレー」タブ・リレー絞り込みメニュー、ロゴメニューの「絵文字申請」、本家Aboutページ・ロゴメニューの「JUICEについて」、未ログイン時トップページの「参加を申請する」・「登録審査状況の確認」)。バッジはJUICEブランドカラー(オレンジ)の塗りつぶしにし、gradateボタン等どんな背景色の上でも視認できるようにした
...
- Feat: JUICE専用Aboutページ(`/about-juice`)に、本家Misskey→misskey-artからの移植→JUICEロードマップの実装→ロードマップ外の追加実装、という機能追加の経路を示す折りたたみ表示を追加

### Server
- Feat: JUICE 独自機能の設定を取得・更新する `admin/juice/settings` / `admin/juice/update-settings` を追加
- Feat: サインイン中のユーザーがメールの受信言語を変更する `i/juice/update-email-lang` を追加
- Feat: 承認式新規登録機能を追加。`admin/juice/pending-signups` / `admin/juice/approve-signup` / `admin/juice/decline-signup` / `juice/public-settings` を追加
- Feat: 承認式新規登録が有効な場合、招待コードを必須から任意に変更。招待コードが有効なら承認をバイパスし、無い場合は理由入力による申請フローに回る
- Feat: システムメール(パスワードリセット・メールアドレス確認・新規登録確認・ログイン通知・アカウント削除通知・モデレーター不在警告・招待制切替通知・通報通知・承認式登録の承認/却下通知)をユーザーごとの言語設定に対応させた。翻訳が無い言語では日本語にフォールバックする
- Enhance: システムメールの言語が ja-JP 以外の場合、翻訳が無いときのフォールバック先を日本語ではなく英語にした(en-US.yml 等のCrowdin管理ファイルは編集できないため、JUICE側で英訳を保持)
- Feat: ノート・Driveファイルに `isAIGenerated` フラグ(JUICE)を追加。`notes/create` / `drive/files/update` で設定でき、`drive/files` の一覧取得で絞り込める
- Feat: `drive/files/create` に `isAIGenerated` パラメータを追加。アップロード後の`drive/files/update`を待たず、アップロード時点でAI生成物フラグを設定できるように
...
- Enhance: `drive/files/delete`が、オブジェクトストレージ利用時に削除をキューへ登録するだけの非同期処理ではなく、実際の削除完了を待ってから応答するように変更(misskey-tempuraの「ドライブファイル即時削除」を参考)。アカウント削除等のバックグラウンド処理は元々同期的だったため、ユーザー操作からの単発削除にもそれと同じ確実性を揃えた
- Fix: サインイン時の承認待ちチェック(承認式新規登録)がパスワード検証より前にあり、パスワードを知らなくてもユーザー名だけで承認待ち状態が判別できた問題を修正。チェックを認証成功後へ移動した(パスキーサインインは元々WebAuthn検証後のため変更なし)
- Fix: `emoji-requests/create`のname/category/aliases/licenseに長さ・件数の上限が無く、DB列の上限を超える入力でバリデーションエラー(400)でなく内部エラー(500)が返っていた問題を修正
- Fix: リレータイムライン(`notes/relay-timeline`/`relayTimeline`チャンネル)がロールポリシーによる制限を受けず、GTLをポリシーで無効化しているサーバーでも公開ノートの閲覧経路として利用できた問題を修正。GTLと同じ`gtlAvailable`ポリシーに従うように
- Fix: 絵文字申請の承認時、絵文字が申請者所有のDriveファイルのURLを直接参照していたため、「審査後にファイルを削除」を有効にした申請の承認や申請者自身によるファイル削除で絵文字画像が表示できなくなる問題を修正。承認時にシステム所有の複製を作成し(`admin/emoji/copy`と同じ`uploadFromUrl`方式)、複製から絵文字を登録するように
- Feat: オブジェクトストレージ上の孤立ファイル(DriveFileから参照されなくなったオブジェクト)を検出し、必要なら削除する管理者向けエンドポイント`admin/drive/find-orphaned-object-storage-files`を追加(JUICE独自実装、misskey-tempuraの「孤立S3ファイルの自動クリーンアップ」を参考)。破壊的かつ不可逆な操作であるため、既定は削除せず一覧を返すのみ(`dryRun`)、権限は`requireAdmin`、アップロード直後(6時間以内に更新された)オブジェクトは孤立候補から除外、削除は1件ずつ失敗を捕捉して継続し結果(`deletedKeys`/`failedKeys`)を返す、モデレーションログに記録、という安全策を設けている。バケット全体を1回で走査するのは大規模インスタンスでは非現実的なため`limit`件数ずつ`continuationToken`でページングする設計にしており、自動実行のcronジョブとしては登録していない(既存の`admin/drive/cleanup`と同様、管理者が手動で呼び出すAPIとしてのみ提供)
...
- Fix: カスタム絵文字がMFMコンテキスト外で描画された際の `injection "Symbol()" not found` 警告を解消 (挙動の変化なし)
