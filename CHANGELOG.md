...
- Enhance: 管理画面の承認待ちユーザー(承認式新規登録)にも、絵文字申請・アバターデコレーション申請と同様に承認済み/却下済みの履歴を閲覧できる状態切り替えを追加(JUICE独自)。却下されたアカウントは物理削除される仕様のため、申請時点のユーザー名・登録理由をスナップショットとして保持し、削除後も履歴から確認できるようにした。既存の承認済みユーザーについても、モデレーションログから審査したモデレーターを遡って補完している
