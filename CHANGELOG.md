...
### Server
- Fix: サーバー名・短縮名にHTMLタグを含む文字列を設定していると、PWAとしてインストールした際のアイコン名(manifest.jsonのname/short_name)やブラウザ検索バーへの追加時(opensearch.xml)のツールチップ等、プレーンテキストとして表示されるべき場所にタグの記号がそのまま表示されてしまう問題を修正。あわせてopensearch.xmlが無加工の文字列結合でXMLを組み立てていた(不正なXMLになりうる)箇所も適切にエスケープするように修正
